...
public class GuiClick extends GuiScreen{
	...
	ArrayList<MEDMEX.myClickGUI.Module> modules = new ArrayList<MEDMEX.myClickGUI.Module>();
	ArrayList<BooleanButton>booleanButtons = new ArrayList<BooleanButton>();
	ArrayList<SliderButton>sliderButtons = new ArrayList<SliderButton>();
	ArrayList<MultiButton>multiButtons = new ArrayList<MultiButton>();
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) 
	{
		...
		for (MEDMEX.myClickGUI.Module m : modules)
		{
			...
			EaglerAdapter.glScalef(1.428577f, 1.428577f, 1);
		...
		for (MEDMEX.myClickGUI.Module m : modules)
		{
			if (m.settings)
			{
				...
				for (SliderButton sb : sliderButtons)
				{
					...
					fr.drawStringWithShadow(doubleToShorterString(sb.s.getValDouble()), (sb.maxX + 6)  * 1.428577f, (sb.minY + 3) * 1.428577f, 0xcccc00ff);
				}
			...
			if (!openSettings)
			{
				if (mouseX >= m.minX && mouseX <= m.maxX && mouseY >= m.minY && mouseY <= m.maxY)
				{
					...
					if (mouseButton == 1)
					{
						...
						keybindString = EaglerAdapter.getKeyName(keybindButton.mod.mod.keyCode.code);
	...
	protected void keyTyped(char typedChar, int keyCode) 
	{
		...
		{
			if(keyCode == 1)
				keyCode = EaglerAdapter.KEY_NONE;
			keybindButton.mod.mod.keyCode.setKeyCode(keyCode);
			keybindString = EaglerAdapter.getKeyName(keyCode);
			listening = false;
			return;
		}

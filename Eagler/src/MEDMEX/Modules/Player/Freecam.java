...
public class Freecam extends Module{
	...
	Entity ent;
	public Freecam() {
		super("Freecam", EaglerAdapter.KEY_NONE, Category.PLAYER);
		instance = this;
	}
	
	public void setup() {
		Client.settingsmanager.rSetting(new Setting("Freecam Speed", this, 1, 1, 5, false));
		Client.settingsmanager.rSetting(new Setting("Fake Player", this, true));
	}
	...
	public void onEnable() {
		...
		if(Client.settingsmanager.getSettingByName("Fake Player").getValBoolean())
		{
	...
	public void onEvent(Event e) {
		if(e instanceof EventUpdate) {
			if(e.isPre()) {
				...
				if(EaglerAdapter.isKeyDown(EaglerAdapter.KEY_SPACE)) {
					mc.thePlayer.motionY = 1 * Client.settingsmanager.getSettingByName("Freecam Speed").getValDouble();
				}
				if(EaglerAdapter.isKeyDown(EaglerAdapter.KEY_LSHIFT)) {
					mc.thePlayer.motionY = -1 * Client.settingsmanager.getSettingByName("Freecam Speed").getValDouble();
				}
				}
